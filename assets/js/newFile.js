...
var tableResarchcount1 = $("#nsrCount1").DataTable({
  ...
  var retailValue = ['1.1', '2.1', '3.1'];
  var costValue = ['1.1', '2.1', '3.1'];
...
var selectedValuesTable = $("#selectedValuesTable").DataTable({
    columns: [
        ...
        { title: "UPC Code", data: 2 },
        { title: "Product Type", data: 3 },
        { title: "Delivery Freq", data: 4 },
        { 
            title: "Retails",
            render: function () {
                return retailsValue; 
...
function updateData() {
    ...
    retailsValue[0] = parseFloat(retail); 
    costValue[0] = parseFloat(cost); 
    
    console.log("Updated Retails Value:", retailsValue);
    console.log("Updated Cost Value:", costValue);
    
   
    selectedValuesTable.rows().invalidate().draw();
  }
  function removeData() {
    const retail = $('#retail').val();
    const cost = $('#cost').val();   
...
$('#addAll').on('click', function(e) {
  ...
  $('.productTitleWork').on('click', function (e) {
    ...
    // Get all checked checkboxes in #nsrCount1 table body
    var checkedRows = $('#nsrCount1 tbody .selectRow:checked');   
    var allDataChanges = [];
    ...
    // If no data changes were collected, set a default value
    if (allDataChanges.length === 0) {
        allDataChanges.push("Please Select a Product");
    }

    // Set the text of .productTitle to the joined values
...
//     // Use `find` on the closest `tr` to get the fifth `td`
//   const fifthCell = closest.find('td:nth-child(6)').text();
//   const sixthCell = closest.find('td:nth-child(7)').text();
//   console.log(fifthCell, sixthCell);
// const changeRetail = $('#retail').val();
// console.log(changeRetail);

//   fifthCell.text(changeRetail);
// });
...
$('#selectedValuesTable tbody').on('click', function (e) {
    ...
    // Use `find` on the closest `tr` to get the first, fifth, and sixth `td`
    const firstCell = closest.find('td:nth-child(1)').text();
    const fifthCell = closest.find('td:nth-child(6)').text();
    const sixthCell = closest.find('td:nth-child(7)').text();
    
    // Check if fifthCell and sixthCell are correctly selected and log them
    console.log(Number(fifthCell), Number(sixthCell));

    // Update the input fields with the values from the fifth and sixth cells
    $('#retail').val(fifthCell);
    $('#cost').val(sixthCell);

    // Update the text of an element with the class 'productTitle' with the value from the first cell
